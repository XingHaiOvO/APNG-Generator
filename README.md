...
本应用（APNG 生成器）按“现状”提供，开发者不对软件作出任何明示或默示的保证，包括但不限于对适销性、特定用途适用性及不侵权的保证。

1. **本地处理**：本应用所有功能均在您的设备本地完成，我们不会收集、上传或存储您导入的任何图片及生成的动图文件。
2. **素材版权**：您应确保导入的图片拥有合法使用权；因使用他人享有版权的素材（如照片、插画、影视截图等）所生成的动图，相关法律责任由您自行承担。
3. **内容责任**：您对使用本应用生成、保存及分享的内容负全部责任，不得用于制作或传播违反法律法规或他人合法权益的内容。
4. **损失限制**：在适用法律允许的最大范围内，开发者不对您因使用或无法使用本应用而产生的任何直接或间接损失（包括数据丢失、文件损坏等）承担责任。
5. **商标声明**：Android、iOS 及相关名称为各自所有者的商标，仅用于说明本应用的运行平台。

安装或使用本应用，即表示您已阅读、理解并同意本声明的全部内容。
